...
//        postInvalidateDelayed(500);
    ...
    public void updateHand(int hour,int min){
        this.hour = hour;
        this.min = min;
    }
    ...
    private void drawHands(Canvas canvas,float hour,float min) {
        Calendar c = Calendar.getInstance();
//        float hour = c.get(Calendar.HOUR_OF_DAY);
        hour = hour > 12 ? hour - 12 : hour;
        drawHand(canvas, (hour + min / 60) * 5f, true);//c.get(Calendar.MINUTE) / 60
        drawHand(canvas, min, false);/*c.get(Calendar.MINUTE)*/
